...
# MERGE-ICON STYLE — SWORD + matching SHIELD cluster, gentle 3/4, fills the square, NO shadow. Anchored to reference/style-sheet.png. Iterate with the operator.
STYLE="A single video-game merge-chain item icon. Rendering: painterly hand-painted look like the reference — soft cel-shading (smooth gradients PLUS crisp highlights), a soft dark PAINTED outline hugging the form, one warm light source from the TOP-LEFT. PROPORTIONS: bold and weighty — a broad blade and a WIDE guard, substantial, NOT thin/elegant and NOT chibi-stubby. COMPOSITION — a PERFECTLY SQUARE 1:1 image showing a SWORD AND a matching SHIELD together as ONE compact cluster. The SHIELD is LARGE — it fills most of the square (centered, and roughly the SAME generous size in EVERY tier) and sits slightly behind. The SWORD crosses diagonally and is ALWAYS FULLY IN FRONT OF the shield (hilt and pommel to the lower-left, blade to the upper-right), sitting clearly ON TOP with its own crisp dark outline. The sword and the shield must READ AS TWO SEPARATE OBJECTS — do NOT let them blend, fuse or merge into each other; keep a clear outline and depth separation between them. Together they FILL the square — about 90% of the frame, only a small even margin, no large empty areas. Use a natural, GENTLE 3/4 view with only MILD perspective — no extreme or exaggerated foreshortening. Both share the SAME material, palette and theme for this tier. The shield SHAPE is given per tier (round for the early tiers, a KITE or TOWER shield for the later tiers). ABSOLUTELY NO shadow, NO drop shadow, NO cast shadow, NO floor, NO ground, NO base, NO pedestal, nothing beneath the objects. Just the sword and its matching shield, no other objects. Keep a LIMITED, cohesive colour palette — a dominant hue family plus wood / metal neutrals, NOT every colour. Effects (glow, embers, runes) ONLY where the subject calls for them, restrained and intrinsic to the item. NO character, NO hands, NO arm, NO creature, NO scene. NO white sticker / die-cut border. NO text, name label, caption, signature, watermark or logo. Isolated on a solid FLAT PURE-WHITE background. Subject: "
