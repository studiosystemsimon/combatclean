...
# Idempotent: skips a class if final/<slug>.png already exists (use FORCE=1 to regen all).
...
CONC="${CONC:-8}"
FORCE="${FORCE:-0}"
mkdir -p "$ROOT/raw" "$ROOT/logs" "$ROOT/final"
...
gen_one() {  # $1=slug  $2=subject
  ...
  for t in 1 2 3 4; do
    ...
    if [ -f "$ROOT/raw/$slug.png" ]; then
      ...
      if [ "$st" = "LIGHT" ]; then cp "$ROOT/raw/$slug.png" "$ROOT/final/$slug.png"; echo "OK   $slug"; return 0; fi
  ...
  if [ -f "$ROOT/raw/$slug.png" ]; then cp "$ROOT/raw/$slug.png" "$ROOT/final/$slug.png"; echo "WARN $slug (kept non-white bg)"; return 0; fi
...
while IFS=$'\t' read -r slug subject; do
  ...
  want "$slug" || continue
  if [ "$FORCE" != "1" ] && [ -f "$ROOT/final/$slug.png" ]; then echo "skip $slug (exists)"; continue; fi
...
echo "final/ count: $(ls "$ROOT/final"/*.png 2>/dev/null | wc -l | tr -d ' ')"
