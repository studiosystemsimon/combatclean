...
gen_one() {  # $1=slug  $2=subject
  ...
  # BOSS rows (subject contains "BOSS") render at ~2x — give them richer detail, overriding the keep-it-simple rule
  case "$subj" in *BOSS*) prompt="${prompt} — THIS IS A BOSS, displayed at roughly 2x the size of a normal enemy: give it RICHER, more ELABORATE and ORNATE detail, more intricate features and a grander, more imposing presence. The 'minimise fine detail / keep it simple' instruction does NOT apply to this boss — it is a large detailed centerpiece." ;; esac
  for t in 1 2 3 4; do
