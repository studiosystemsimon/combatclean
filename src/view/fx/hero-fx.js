...
function burstInto(host) {
  ...
  f.animate([{ opacity: 0 }, { opacity: 1, offset: 0.2 }, { opacity: 0 }], { duration: 420, easing: 'ease-out' }).onfinish = () => f.remove();
  const rays = document.createElement('div'); rays.className = 'lvfx-rays'; host.appendChild(rays);
  rays.animate([{ opacity: 0, transform: 'translate(-50%,-50%) rotate(0deg) scale(.4)' }, { opacity: 0.85, transform: 'translate(-50%,-50%) rotate(40deg) scale(1)', offset: 0.3 }, { opacity: 0, transform: 'translate(-50%,-50%) rotate(95deg) scale(1.3)' }], { duration: 700, easing: 'ease-out' }).onfinish = () => rays.remove();
  for (let i = 0; i < 2; i++) { const r = document.createElement('div'); r.className = 'lvfx-ring'; host.appendChild(r);
    r.animate([{ transform: 'translate(-50%,-50%) scale(.3)', opacity: 1 }, { transform: 'translate(-50%,-50%) scale(5)', opacity: 0 }], { duration: 520 + i * 150, delay: i * 100, easing: 'cubic-bezier(.2,.7,.3,1)' }).onfinish = () => r.remove(); }
  for (let i = 0; i < 14; i++) { const s = document.createElement('div'); s.className = 'lvfx-spark'; host.appendChild(s);
    const a = (i / 14) * Math.PI * 2 + Math.random() * 0.4, d = 26 + Math.random() * 40;
    s.animate([{ transform: 'translate(-50%,-50%) translate(0,0) scale(1)', opacity: 1 }, { transform: `translate(-50%,-50%) translate(${Math.cos(a) * d}px,${Math.sin(a) * d}px) scale(.3)`, opacity: 0 }], { duration: 560 + Math.random() * 200, easing: 'cubic-bezier(.2,.7,.3,1)' }).onfinish = () => s.remove(); }
}
function particleBurst(cx, cy, big) {
  const p = portal();
  const n = big ? 34 : 15, base = big ? 55 : 28, spread = big ? 150 : 70, size = big ? 8 : 5;
  for (let i = 0; i < n; i++) { const s = document.createElement('div'); s.className = 'pburst'; s.style.left = cx + 'px'; s.style.top = cy + 'px'; s.style.width = size + 'px'; s.style.height = size + 'px'; p.appendChild(s);
    ...
    s.animate([{ transform: 'translate(-50%,-50%) scale(1)', opacity: 1 }, { transform: `translate(-50%,-50%) translate(${Math.cos(a) * d}px,${Math.sin(a) * d}px) scale(.2)`, opacity: 0 }], { duration: (big ? 900 : 620) + Math.random() * 260, easing: 'cubic-bezier(.15,.7,.3,1)' }).onfinish = () => s.remove(); }
  if (big) { const ring = document.createElement('div'); ring.className = 'pburst-ring'; ring.style.left = cx + 'px'; ring.style.top = cy + 'px'; p.appendChild(ring);
    ring.animate([{ transform: 'translate(-50%,-50%) scale(.2)', opacity: 1 }, { transform: 'translate(-50%,-50%) scale(10)', opacity: 0 }], { duration: 720, easing: 'cubic-bezier(.2,.7,.3,1)' }).onfinish = () => ring.remove(); }
}
...
function tween(node, from, to, ms, f) {
  ...
  node.animate([{ transform: 'scale(1)' }, { transform: 'scale(1.5)', offset: 0.3 }, { transform: 'scale(1)' }], { duration: ms, easing: 'ease-out' });
}
function maxedOut(tileEl) {
  ...
  const typer = setInterval(() => { i++; d.textContent = word.slice(0, i);
    d.animate([{ transform: 'translate(-50%,-50%) rotate(-4deg) scale(1.1)' }, { transform: 'translate(-50%,-50%) rotate(4deg) scale(1.02)', offset: 0.5 }, { transform: 'translate(-50%,-50%) rotate(0deg) scale(1)' }], { duration: 110, easing: 'ease-out' });
    if (i >= word.length) { clearInterval(typer);
      setTimeout(() => { d.animate([{ transform: 'translate(-50%,-50%) scale(1)', opacity: 1 }, { transform: 'translate(-50%,-50%) scale(0)', opacity: 0 }], { duration: 560, easing: 'cubic-bezier(.7,0,.84,0)' }).onfinish = () => d.remove(); }, 3000);
    }
  }, 55);
}
...
export function fxHeroLevelUp(tileEl, { fromLv, toLv, fromPow, toPow, cap }) {
  ...
  tileEl.animate([{ transform: 'scale(1)' }, { transform: `scale(${multi ? 1.2 : 1.16})`, offset: 0.3 }, { transform: 'scale(.97)', offset: 0.6 }, { transform: 'scale(1)' }], { duration: multi ? 620 : 520, easing: 'cubic-bezier(.2,1.4,.3,1)' });
  const g = getComputedStyle(tileEl).getPropertyValue('--rar');
  tileEl.animate([{ boxShadow: `0 0 0 0 ${g}` }, { boxShadow: `0 0 ${multi ? 26 : 20}px ${multi ? 7 : 5}px ${g}`, offset: 0.3 }, { boxShadow: '0 3px 9px rgba(0,0,0,.45)' }], { duration: multi ? 820 : 680, easing: 'ease-out' });
  burstInto(fxwrap);
  const c = centerOf(tileEl);
  particleBurst(c.x, c.y, multi);
  tween(powNode, fromPow, toPow, multi ? 900 : 720, fmt);
  if (lvNode) { tween(lvNode, fromLv, toLv, multi ? 900 : 500); setTimeout(() => { lvNode.innerHTML = '<s>LV</s>' + toLv; }, multi ? 920 : 520); }
  const stats = [{ t: '+' + Math.max(1, Math.round(gain * 0.13)) + ' ATK', c: 'atk' }, { t: '+' + Math.max(1, Math.round(gain * 0.9)) + ' HP', c: 'hp' }, { t: '+' + gain + ' PWR', c: 'pwr' }];
  if (!multi) {
    floatText(c.x, c.y - 4, 'LEVEL UP!', 'hd', 0);
    stats.forEach((s, i) => floatText(c.x + (i - 1) * 3, c.y, s.t, 'stat ' + s.c, 210 + i * 160));
    if (toLv >= cap) setTimeout(() => maxedOut(tileEl), 320);
    return;
  }
  const stagger = Math.max(55, Math.min(150, Math.round(900 / levels)));
  ...
  stats.forEach((s, i) => floatText(c.x + (i - 1) * 3, c.y, s.t, 'stat ' + s.c, afterTexts + i * 160, 2600)); // one total set, 1s longer
...
export function fxLevelAll(tileEl, entries, fromPow, toPow) {
  if (!tileEl) return;
  tween(tileEl.querySelector('.hs-pow b'), fromPow, toPow, 720, fmt);
  const c = centerOf(tileEl);
  particleBurst(c.x, c.y, entries.some((e) => e.levels > 1));
  entries.forEach((e, i) => setTimeout(() => {
    const s = e.slotEl;
    if (!s) return;
    s.animate([{ transform: 'scale(1)' }, { transform: 'scale(1.22)', offset: 0.35 }, { transform: 'scale(1)' }], { duration: 420, easing: 'cubic-bezier(.2,1.4,.3,1)' });
    burstInto(fxHost(s));
    const lv = s.querySelector('.hs-glv'); // the slot shows its LEVEL; count it up
    if (lv) tween(lv, e.fromLevel, e.toLevel, e.levels > 1 ? 720 : 520);
    const r = centerOf(s);
    const gain = e.toPow - e.fromPow;
    if (e.levels > 1) {
      const stagger = Math.max(45, Math.min(120, Math.round(700 / e.levels)));
      for (let k = 0; k < e.levels; k++) floatText(r.x, r.y - 6, 'LEVEL UP', 'lvup', k * stagger);
      floatText(r.x, r.y, '+' + gain + ' PWR', 'stat pwr', e.levels * stagger + 120, 2600); // one total, held 1s longer
    } else {
...
export function fxEquipBest(tileEl, slotEls, fromPow, toPow) {
  ...
  slotEls.forEach((s, i) => setTimeout(() => {
    s.animate([{ transform: 'scale(1)' }, { transform: 'scale(.68) rotate(-8deg)', offset: 0.3 }, { transform: 'scale(1.2) rotate(5deg)', offset: 0.72 }, { transform: 'scale(1)' }], { duration: 460, easing: 'cubic-bezier(.2,1.25,.3,1)' });
  ...
  const after = Math.max(0, slotEls.length - 1) * 230 + 220;
  setTimeout(() => { if (powNode) tween(powNode, fromPow, toPow, 520, fmt); const c = centerOf(tileEl); particleBurst(c.x, c.y, false); }, after);
}
...
export function fxEquip(slotEl, tileEl, fromPow, toPow) {
  ...
  setTimeout(() => {
    if (powNode) tween(powNode, fromPow, toPow, 520, fmt);
...
export function fxHeroFuse(tileEl, count, color, onCommit) {
  ...
  spots.forEach((s, i) => {
    ...
    clone.style.cssText = `position:fixed; left:${tx + s.dx}px; top:${ty + s.dy}px; width:46px; height:46px; object-fit:contain; display:grid; place-items:center; font-size:30px; z-index:9998; pointer-events:none; margin:0;`;
  ...
  const landMs = 440 + (spots.length - 1) * 70 + 40;
  setTimeout(() => {
    onCommit();
    fx.flash(0.85, 130);
    fx.shake(9);
    const c = fx.elCenter ? fx.elCenter(tileEl) : { x: tx, y: ty };
    if (c) fx.impact(c.x, c.y, { tier: 'crit', color, r: 18 });
